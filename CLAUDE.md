...
- **A restart is a stream killer, so it must drain first.** `docker restart` cuts every in-flight SSE mid-body and each client reports `Connection lost mid-response` — that agent turn is lost. The hub carries **6-9 concurrent streams at any instant** (measured 2026-08-23), so a blind restart costs 6-9 turns. `/health` reports `activeStreams` and `uptimeSec` (PR #37) precisely so a restart can wait; `scripts/claudish-drain.ps1` is the wrapper that does it. **`ClaudishDailyRestart` (04:00) runs the wrapper since 2026-08-23** (first drained restart: 08-24 04:00, logged in `~/.claudish/drain.log`). A `$null` count means *no signal* (proxy down, or image predates #37) and must degrade to an undrained restart, never to an unbounded wait.
