...
| `compress-captures.ps1` · `claudish-watchdog.ps1` | nightly 7z compaction + GDrive backup (02:47 — moved off 04:17 on 2026-08-20: the 60-70 min run collided with the 04:00 container restart) · health check, proactive restart, hang recovery. **The watchdog has NO scheduled task** (verified 2026-08-23: log stops at its 2026-05-31 manual creation runs; only `ClaudishDailyRestart` docker restart 04:00 and `ClaudishCaptureCompaction` 02:47 exist). Since #37 it classifies probe failures (429/402/529 → DEGRADED, no restart), requires 2 consecutive hang cycles, drains via `/health` `activeStreams` before restarting, and only restarts proactively in the 03:00-06:00 quiet window — safe to schedule if hang detection is wanted |
