 ...
 */
export async function initStats(config: ClaudishConfig): Promise<void> {
  try {
...
export function recordStats(partial: Partial<StatsEvent>): void {
  try {
    ...
    // Check if it's time for a flush (24h interval) in background
    checkAndFlush().catch(() => {});
  } catch {
    // Never crash claudish
  }
 ...
 */
export async function showMonthlyBanner(): Promise<void> {
  try {
    if (isStatsDisabledByEnv()) return;
