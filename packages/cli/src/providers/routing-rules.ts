 ...
 *
 * Catalog routes describe availability and wire IDs. They do not replace
 * routing preferences: bundled rules, then global and local overrides decide
 * order. The model and plan snapshot is consulted when building candidates.
 *
...
/**
 * Keep only catalog routing entries whose provider has an executable transport.
 */
